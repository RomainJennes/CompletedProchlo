...
class Crypto {
 public:
  ...
  bool load_analyzer_private_key(const std::string& keyfile);

  
  bool load_blinder_private_key(const std::string& keyfile);

  
  bool load_thresholder_private_key(const std::string& keyfile);
  ...
  bool DecryptBlinder(const BlinderItem& blinder_item,
    PlainBlinderItem* plain_blinder_item);
  bool DecryptThresholder( const ThresholderItem& thresholder_item,
    PlainThresholderItem* plain_thresholder_item);
  bool DecryptAnalyzer(const AnalyzerItem& analyzer_item,
  ...
  EVP_PKEY* private_blinder_key_;
  EVP_PKEY* private_thresholder_key_;
  EVP_PKEY* private_analyzer_key_;
 ...
 private:
  ...
  class Decryption {
   public:
    Decryption(EVP_PKEY* peer_key) : peer_key(peer_key) {}

    EVP_PKEY* ToPeerKey() { return peer_key; }

    virtual bool StreamDataForDecryption(EVP_CIPHER_CTX* ctx) = 0;
    virtual EVP_PKEY* ToPrivateKey() = 0;
  ...
  class PlainAnalyzerItemToAnalyzerItemDecryption : public Crypto::Decryption {
   public:
    ...
    EVP_PKEY* ToPrivateKey()override;
    const char* TypeString() override;
    uint8_t* ToNonce() override;
    uint8_t* ToTag() override;
    bool StreamDataForDecryption(EVP_CIPHER_CTX* ctx) override;
  };
  class PlainThresholderItemToThresholderItemDecryption : public Crypto::Decryption {
   public:
   ...
   class PlainBlinderItemToBlinderItemDecryption : public Crypto::Decryption {
   public:
  ...
  bool MakeDecryptedMessage(Decryption* decryption);
  ...
  bool Decrypt(const uint8_t* symmetric_key, Decryption* decryption);
